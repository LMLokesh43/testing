 ...
 */
export const getNameListString = (ids: string[], userDictionary: { [id: string]: IUser}, separator?: string): string => {
    if (!ids || ids.length === 0) {
        return "";
    } else {
...
export const getAllNames = (userDictionary: { [id: string]: IUser}): string[] => {
    if (!userDictionary) {
        return [];
    }
    return Object.values(userDictionary).map((user: IUser): string => user.name);
};
 ...
 */
export const getMentionedUserId = (content: string, userDictionary: { [id: string]: IUser}): string [] => {
    if (!content) {
        return [];
    }
    const ids: string[] = [];
    const userArray: IUser[] = Object.values(userDictionary);
    ...
    if (results) {
        results.forEach((result: string) => {
            ...
            const found: number = userArray.findIndex((user: IUser) => user.name === name);
            if (found >= 0) {
